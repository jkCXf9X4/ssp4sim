...
9. **Hot path logging**:
   ...
   * `ssp4cpp::utils::log::add_cutelog_sink(host, port, level)` sends length-prefixed JSON records to a cutelog TCP listener. Start cutelog first, then configure this sink before constructing loggers.
