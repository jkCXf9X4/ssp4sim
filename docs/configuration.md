...
| `simulation.executor.la2.iterations` | `int` | No | SCC node count | Number of internal sub-steps the `la2` executor takes per macro step inside each loop SCC. Defaults to the SCC size (`1` = no sub-step relaxation). Nested (loop-within-loop) SCCs need a higher count because their feedback path passes through the inner loop nodes as well; the relaxation rate (Jacobi spectral radius) tightens with more iterations. Legacy alias: `simulation.executor.loop_aware.iterations` is still honored as a fallback. |
| `simulation.executor.la2.mode` | `string` | No | `linear` | How the macro step is subdivided for a loop SCC: `linear` = `iterations` equal sub-steps; `factor` = sub-steps shrink by `simulation.executor.la2.factor`, so the smallest sub-steps land at the macro-step end. Both modes only advance time (models cannot be reset), so the loop relaxes as a moving wavefront toward the macro-step boundary. The scheduling guarantees the macro interval `[0, end]` is fully covered (the last sub-step covers the tail). Legacy aliases: `fixed` → `linear`, `geometric` → `factor`; `simulation.executor.loop_aware.mode` / `.factor` are still honored as fallbacks. |
...
| `simulation.recording.sqlite.enable` | `bool` | No | `false` | Enables the SQLite WAL recorder for live-concurrent-reader access. When `simulation.recording.sqlite.file` is absent (default), SQLite WAL writes one database file per simulation run, named `{epoch_seconds}_{session_uuid}.sqlite`, in the working directory — this is the safe default for concurrent simulations. When `sqlite.file` is set, the sink writes to that specific path instead (sequential-only — see safety table below). Within each file, every signal storage creates a separate table named `{run_id}_{model}_{storage_name}` storing `timestamp_ns`, `simulation_time_s`, and the storage variables. WAL journal mode allows concurrent readers during writes. The writer batches 10,000 inserted rows per transaction, uses `PRAGMA synchronous=NORMAL`, and defers WAL checkpointing until shutdown to reduce commit overhead; transactions remain atomic, but live readers see data in larger batches and the last committed transactions can be less durable than SQLite `FULL` synchronous mode if the host loses power. |
