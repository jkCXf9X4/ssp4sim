...
| `simulation.executor.la2.parallel` | `bool` | No | `false` | If `true`, the `la2` outer Gauss-Seidel executor is `ParallelSeidel` instead of `SerialSeidel`. `ParallelSeidel` is **not implemented** yet, so this throws at `la2` construction until the stub lands. |
