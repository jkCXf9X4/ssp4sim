...
- **Recursive building** — `SspSystemBuilder` walks nested `<ssd:System>` elements.

## Files

| File | Purpose |
|------|---------|
| `analysis_system.hpp` / `.cpp` | `SspSystem` container with recursive model/connection collection |
...
auto sys = analysis::SspSystemBuilder().build("path/to/ssp.ssp");
