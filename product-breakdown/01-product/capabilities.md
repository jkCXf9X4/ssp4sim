...
| CAP-005 | Local Database Recording | Record simulation results to a local database with per-storage tables. Current backend: DuckDB. **Concurrent-writer safety**: Only per-simulation-file SQLite WAL supports parallel execution. CSV, DuckDB, and shared-file SQLite are unsafe for concurrent writers. |
