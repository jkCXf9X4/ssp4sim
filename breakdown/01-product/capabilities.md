...
| CAP-005 | Local Database Recording | Record simulation results to a local database with per-storage tables. Current backend: SQLite WAL. **Concurrent-writer safety**: Only per-simulation-file SQLite WAL supports parallel execution. CSV and shared-file SQLite are unsafe for concurrent writers. |
...
- Backward: Traces to intent outcomes in `breakdown/00-intent/outcomes.md`.
