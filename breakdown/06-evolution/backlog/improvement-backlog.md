...
| `done/IMP-010.md` | IMP-010 | Remove logging guidelines from PBS signal.md — domain mismatch | Done | High | 2 files (PBS signal.md, docs/logging_guidlines.md); text removal only |
...
| `done/IMP-012.md` | IMP-012 | Consolidate "ctest is unreliable" note to one canonical location | Done | Medium | 3 files (docs/build_from_source.md, tests/README.md, PBS test-strategy.md); minor text changes |
...
| `done/IMP-017.md` | IMP-017 | Deduplicate test pyramid description between tests/README.md and PBS strategy | Done | Low | 2 files (tests/README.md, PBS test-strategy.md); minor text changes |
...
| `done/IMP-033.md` | IMP-033 | Refactoring & Cleanup of Graph Builder Files | Done | Medium | analysis_graph_builder.cpp/.hpp, graph_builder.cpp/.hpp, test files; low-medium risk per finding |
| `done/IMP-034.md` | IMP-034 | Strip DuckDB result feature | Done | High | 5 source files deleted, 1 test deleted, 2 Python scripts deleted, build system (3 files), config (2 files), 30+ doc artifacts; large blast radius but mechanical removal |
...
Items moved to `deferred/` — do **not** implement unless explicitly and well-motivated told to do so.

| File | ID | Theme | Status | Priority | Blast radius |
| --- | --- | --- | --- | --- | --- |
| `deferred/IMP-021.md` | IMP-021 | Implement a working ParallelSeidel DAG scheduler | Deferred | High | seidel_parallel.hpp/cpp, seidel_base.hpp, executor_builder.cpp, test file; moderate blast radius, data race risk if atomics omitted |
| `deferred/IMP-026.md` | IMP-026 | Direct rerun (pipelined) Gauss-Seidel executor | Deferred | Low | No code changes; design document only; highly speculative, FMI-incompatible |
